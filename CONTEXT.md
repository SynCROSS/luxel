...
**luxel-core**: Rust crate (`crates/luxel-core`) — Luxel SFC parse/IR, compiled SSR execution, and (phase 3) bundling. Consumed from JS via napi-rs (`@luxel/core-node`) / WASM / Deno FFI; not a separate product surface. **Phase 1 tracer (2026-06-08):** `renderSpiralRouteFromStore(snapshotJson)` native spiral SSR parity-tested vs TS `renderRouteDocumentFromStore`. Manifest `ssr: "native"` + `luxel.config.ts` `routes[path].ssr` delegate spiral `renderFromStore` to luxel-core (TS fallback if addon missing). `examples/spiral` defaults native.

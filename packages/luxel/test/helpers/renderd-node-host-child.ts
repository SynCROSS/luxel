...
try {
  ...
  if (!html.includes('id="wrapper"')) {
    throw new Error("renderd spiral missing wrapper under node host");
  }
  if (html.match(/class="tile"/g)?.length !== spiralTileCount()) {
    throw new Error("renderd spiral tile count mismatch under node host");
  }
  console.log("renderd-node-host:ok");
} finally {
  await client.close();
}

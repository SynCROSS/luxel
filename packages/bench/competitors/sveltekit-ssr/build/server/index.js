import { U as UNDEFINED, N as NAN, P as POSITIVE_INFINITY, a as NEGATIVE_INFINITY, b as NEGATIVE_ZERO, S as SPARSE, i as is_valid_array_len, M as MAX_ARRAY_INDEX, c as is_valid_array_index, H as HOLE, D as DevalueError, d as is_primitive, g as get_type, e as is_plain_object, f as enumerable_symbols, s as stringify_key, h as stringify_string, v as valid_array_indices, w as with_request_store, u as uneval, j as s, k as ur, l as U$1, R, T as T$1, C as C$1, q as q$1, z as z$1, A, I, O, m as i$2, n as a$1, o as a$2, L as L$1, $, p as j, r as f$1 } from './chunks/exports-DUa-NqkE.js';
...
`},version_hash:"1290n06"};async function v(){return {handle:void 0,handleFetch:void 0,handleError:void 0,handleValidationError:void 0,init:void 0,reroute:void 0,transport:void 0}}
...
`);if(A){const R=[];y.style_needs_nonce&&R.push(`nonce="${y.nonce}"`),y.add_style(A),k.add_style(A,R);}for(const R of p){const j=q(R),H=['rel="stylesheet"'];S.has(R)?H.push("disabled",'media="(max-width: 0)"'):u.preload({type:"css",path:j})&&$$1.add(`<${encodeURI(j)}>; rel="preload"; as="style"; nopush`),k.add_stylesheet(j,H);}for(const R of w){const j=q(R);if(u.preload({type:"font",path:j})){const H=R.slice(R.lastIndexOf(".")+1);k.add_link_tag(j,['rel="preload"','as="font"',`type="font/${H}"`,"crossorigin"]),$$1.add(`<${encodeURI(j)}>; rel="preload"; as="font"; type="font/${H}"; crossorigin; nopush`);}}const P=tt(r$1),{data:G,chunks:J}=m.get_data(y);if(a.ssr&&a.csr&&(T+=`
			...
			`)}`),a.csr){const R=s._.client.routes?.find(O=>O.id===c$1.route.id)??null;if(l.uses_env_dynamic_public&&n.prerendering&&h.add(`${i$1}/env.js`),!l.inline){const O=Array.from(h,z=>q(z)).filter(z=>u.preload({type:"js",path:z}));for(const z of O)$$1.add(`<${encodeURI(z)}>; rel="modulepreload"; nopush`),r$1.preload_strategy!=="modulepreload"?k.add_script_preload(z):k.add_link_tag(z,['rel="modulepreload"']);}if(s._.client.routes&&n.prerendering&&!n.prerendering.fallback){const O=Nt(c$1.url.pathname);n.prerendering.dependencies.set(O,Zt(R,c$1.params,new URL(O,c$1.url),s));}const j=[],H=l.uses_env_dynamic_public&&n.prerendering,re=[`base: ${d}`];if(t&&re.push(`assets: ${N(t)}`),l.uses_env_dynamic_public&&re.push(`env: ${H?"null":N(i)}`),J){j.push("const deferred = new Map();"),re.push(`defer: (id) => new Promise((fulfil, reject) => {
						...
						${re.join(`,
						`)}
					};`);const C=["element"];if(j.push("const element = document.currentScript.parentElement;"),a.ssr){const O={form:"null",error:"null"};x&&(O.form=Hr(x,c$1.route.id,r$1.hooks.transport)),i$2&&(O.error=uneval(i$2));const z=[`node_ids: [${e$1.map(({node:Z})=>Z.index).join(", ")}]`,`data: ${G}`,`form: ${O.form}`,`error: ${O.error}`];if(o$1!==200&&z.push(`status: ${o$1}`),s._.client.routes){if(R){const Z=Yt(R,c$1.url,s).replaceAll(`
`,`
...
${ne}	${z.join(`,
${ne}	`)}
${ne}}`);}const{remote:X}=f;let le="",be="";if(X.data){const O={},z={};for(const[Z,Ze]of X.data)if(Z.id)for(const et in Ze){const Ae=Ze[et];if(!Ae.serialize)continue;const $e=He(Z.id,et),tt=Z.type==="prerender"?z:O;if(f.remote.refreshes?.has($e)||f.remote.reconnects?.has($e))tt[$e]=await Ae.data;else {const ve=await Promise.race([Promise.resolve(Ae.data).then(ue=>({settled:true,value:ue}),ue=>({settled:true,error:ue})),new Promise(ue=>{queueMicrotask(()=>ue({settled:false}));})]);if(ve.settled){if("error"in ve)throw ve.error;tt[$e]=ve.value;}}}const ne=ft$1(r$1.hooks.transport);Object.keys(O).length>0&&(le=`${P}.query = ${uneval(O,ne)};

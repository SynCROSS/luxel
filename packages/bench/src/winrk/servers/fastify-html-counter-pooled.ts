...
import { createFastifyHtmlCounterRenderPool } from "@luxel/luxel/bench";
...
export async function startFastifyHtmlWorkerPoolServer(): Promise<BenchServer> {
  const pool = createFastifyHtmlCounterRenderPool();
  await pool.warmup();

...
/** Tier-2 spiral: winrk histogram only — post-winrk sample adds minutes per stack on Windows. */
export function benchLatencySampleCountForFixture(
  fixture: "counter" | "spiral",
): number {
  if (fixture === "spiral") {
    const fromEnv = parseNonNegativeInt(process.env.BENCH_LATENCY_SAMPLES);
    if (fromEnv !== undefined) return fromEnv;
  ...
  return benchLatencySampleCount();
}
...
export async function recoverLocalhostSockets(
  url: string,
  cooldownMs = postWinrkCooldownMs(),
): Promise<void> {
  if (cooldownMs > 0) {
...
export async function prepareForWinrkMeasurement(
  url: string,
  cooldownMs?: number,
): Promise<void> {
  await recoverLocalhostSockets(url, cooldownMs ?? postWinrkCooldownMs());
}

export function winrkPreMeasurementCooldownMsForFixture(
  fixture: "counter" | "spiral",
): number | undefined {
  if (fixture === "spiral" && process.env.BENCH_POST_WINRK_COOLDOWN_MS === undefined) {
    return 1000;
  }
  return undefined;
}

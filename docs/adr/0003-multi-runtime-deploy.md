...
Luxel is **Vite-free** and today centers on **Bun** for `luxel build`, `luxel dev`, bundling (`Bun.build`), and local servers (`Bun.serve`). Production deploy should not require Bun on the host. This ADR locks **phase B** (pre-v1.0): run the app server on **Node** and **Deno** while the **toolchain stays Bun-only** through **v1.0**. **Phase A (v1.1):** full toolchain parity — `luxel dev` / `luxel build` / `luxel bench` / framework tests on Bun, Node, and Deno.

## Decision

1. **Phasing:**
   - **Phase B (pre-v1.0):** production server on Node 20+ and Deno 2+; `luxel build` / `luxel dev` / `luxel bench` / framework unit tests remain **Bun-only**.
   - **v1.0:** unchanged from phase B for **toolchain** (authors still build and develop on Bun); **deploy** on Node/Deno remains in scope for the v1.0 app-framework release (see root `CONTEXT.md` **v1.0 release scope**).
   - **Phase A (v1.1):** toolchain parity on Bun, Node, and Deno.
...
| Topic | Rejected | Why |
|-------|----------|-----|
| v1.0 toolchain parity | Full toolchain on Bun, Node, Deno at v1.0 (A) | v1.0 targets app-framework completeness first; toolchain parity moves to **v1.1** (phase A). |
| v1 scope day one | Full toolchain parity on Bun, Node, Deno at first stable tag (A) | Deepest lock-in is `Bun.build`; ship deploy first (B), then app features at v1.0, then toolchain at v1.1. |
| Deploy only | CLI stays Bun forever | Toolchain parity is explicit at **v1.1** (phase A); not deferred indefinitely. |
...
4. **Integration test** — Node and **Deno** (CI-required) spawn adapters against built counter and nav-demo `dist/`.

Also: **deploy matrix** doc table (runtime × toolchain × zstd availability).

### Deferred

- **v1.1** toolchain parity (dev/build/bench on Node and Deno).
...
- `docs/architecture.md` §1 Runtime row reflects phase B, v1.0 deploy, and v1.1 toolchain parity.
